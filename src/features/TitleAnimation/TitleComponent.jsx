...
export default function TitleComponent({ text, nameClass = '' }) {
	...
	return (
		<div ref={container} className={`section-title${text.split(" ").length === 1 ? nameClass + " word" : nameClass}`}>
			{
				title.map((word, index) => {
					return <motion.span key={index} variants={slideUp} custom={index} initial="initial" animate={isInView ? "open" : "closed"}>
						{word}
					</motion.span>
...
// 	return (
// 		<div ref={container} className={`section-title${nameClass}`}>
// 			{
// 				title.map((word, index) => {
